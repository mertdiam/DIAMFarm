...
- 2026-07-12: Phase 0 code audit complete (docs/internal/audit-findings.md). Code-side verdict: GO, fork and keep. 6 confirmed Criticals (2 in deferred Elegoo drivers; 4 in set-ready/scheduler/operator endpoints get a dedicated fix brief before launch). Final go/no-go still gated on Task 0b hardware validation.
- 2026-07-12: PR 2 (Better Auth) merged. Decisions: auth tables are excluded from the JSON backup export (session tokens and password hashes do not belong in a downloadable bundle; farm.db file copy is the DR path for auth). Cross-family review was unavailable in this environment; substituted an independent adversarial Claude review in a separate session, which found no bypass and two should-fixes (fixed pre-merge). legacy-peer-deps pinned via .npmrc until better-sqlite3 is bumped from 9.x.
- 2026-07-12: Phase 1 code complete. PRs 1-4 all merged (scheduler regression suite, Better Auth, credential redaction, header parser). Suite at 30/457 green. Remaining before launch: Task 0b hardware validation (human), TASK 6 audit-criticals fix pack (brief in TASKS.md, gated on Mert sign-off per completed_qty escalation rule), real slicer fixture files (human).
- 2026-07-12: TASK 6 audit-criticals fix pack merged (PR 6) with Mert sign-off per the completed_qty escalation rule. set-ready gained an is_held 409 precondition, transactional single-use-hold idempotency, and a newer-job scope guard on the session-failed fallback; the scheduler consults _activeUploads before the stale-job auto-fail and holds untracked FINISHED printers instead of dispatching. Adversarial review verdict MERGE, all axes clean. Suite at 32/473. Repo renamed to github.com/mertdiam/DIAMFarm the same day.
- 2026-07-12: Deployed to Coolify (app uuid dduqhb3h75zhw7f6sblwjjtn, project DIAMFarm, http://diamfarm.100.105.233.92.sslip.io). Deployment path is a PREBUILT image: the fork-image workflow publishes ghcr.io/mertdiam/diamfarm:production (runtime target) on every production push, and the Coolify app is dockerimage type pulling that tag. On-host git builds fail on the diamcore server in under a minute with no build-log API to diagnose; canary apps proved the host builds dockerfile-content apps fine (including a native better-sqlite3 compile), so the failure is specific to Coolify's git build path and was bypassed rather than diagnosed. Redeploy after a merge: trigger the fork-image workflow (automatic on push), wait for it to go green, then POST /api/v1/deploy?uuid=dduqhb3h75zhw7f6sblwjjtn&force=true against 192.168.45.3:8000 with the Coolify API token (in the coolify MCP server config). CI-triggered auto-deploy is parked until the Tailscale funnel (diamcore.corn-codlet.ts.net, currently NXDOMAIN) is restored; the COOLIFY_API_TOKEN repo secret is already set for that day. Volumes diamfarm-sqlite (/app/server/data) and diamfarm-gcode (/app/server/gcode) persist state. Fixed en route: Dockerfile now copies .npmrc before npm ci (real ERESOLVE bug), and Coolify needs dockerfile_target_build=runtime (default builds the dev stage). GitHub default branch switched to production (required for workflow_dispatch; matches PR flow).
- 2026-07-12: Tailnet renamed orfe-gamma -> corn-codlet, but the funnel diamcore.corn-codlet.ts.net still returns NXDOMAIN from Tailscale's own resolver (verified via workflow fan-out), so CI auto-deploy stays PARKED. When the funnel resolves, add a deploy job to .github/workflows/fork-build-check.yml: needs build-push, if github.event_name=='push' && github.ref=='refs/heads/production', one step `curl -fsS -X POST "https://diamcore.corn-codlet.ts.net/api/v1/deploy?uuid=dduqhb3h75zhw7f6sblwjjtn&force=true" -H "Authorization: Bearer ${{ secrets.COOLIFY_API_TOKEN }}"`. Security note from review: the funnel would expose Coolify's deploy API publicly; scope the funnel to the deploy path if possible and rotate the token. Also hardened the image this day: non-root farmapp user (uid 10001) via a gosu drop-privileges entrypoint that chowns the root-owned volumes at start (no host action needed), plus a /api/health HEALTHCHECK.
