-- Better Auth schema reference (generated, do NOT run at deploy time).
--
-- Produced once during development with:
--   npx auth@1.6.23 generate --config server/auth.mjs
-- for better-auth@1.6.23 with the admin plugin enabled.
--
-- This file is a reference artifact only. The live tables are created by
-- server/db.js as CREATE TABLE IF NOT EXISTS statements (the house migration
-- pattern), so no Better Auth CLI runs on deploy. If you upgrade better-auth or
-- change auth plugins, regenerate this file and reconcile the db.js block.
