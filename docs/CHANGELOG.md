...
**Finding 4, set-ready phantom credit (server/index.js).** The set-ready handler had no `is_held` precondition and inferred what to credit purely from job rows, so a stale Fleet tab or a duplicate submission could credit an old cancelled job or an in-flight upload. Real-world trigger: operator A releases a held printer via batch Set Ready and the scheduler dispatches a new job (status `uploading`); operator B's stale tab still shows the printer as held and clicks Set Ready, crediting the wrong job and possibly closing the part and cancelling its queued work. Fix (a): the endpoint now returns `409 { error: 'Printer is not awaiting sign-off' }` before any state change unless `is_held = 1`. Fix (b): the session-failed fallback (the MQTT-reconnect recovery path) now additionally requires that no newer job exists for the printer, so a stale `failed` row left by an earlier dispatch can never be credited by a later unrelated Set Ready.

**Finding 5, set-ready non-idempotent confirmed_qty (server/index.js).** The confirmed_qty delta was reapplied on every request with no marker that the job was already adjusted, so a double-click subtracted twice, and a cross-path duplicate (missed-finish credit followed by a normal-finish delta) corrupted the count further. Real-world trigger: an operator double-clicks Set Ready, or a client retries after a network timeout; the Fleet button had no in-flight guard. Fix: all crediting plus the hold release now run in one transaction with the hold re-checked at the top, making `is_held` a single-use token for one sign-off. The first request credits and releases the hold; every duplicate then hits the 409 precondition. `completed_qty` changes exactly once across any number of duplicate submissions.

**Finding 6, stale-job auto-fail races a live upload (server/scheduler.js).** In `_dispatchToPrinter` the stale-job auto-fail block ran before the `_activeUploads` in-flight-upload guard. A real upload cycle routinely outlasts `STALE_JOB_GRACE_MS` (the 60s UPLOAD_CONFLICT wait plus multi-minute large-file transfers), so a dispatch firing during the upload (reachably via the unguarded poller `printerIdle` listener during a poll flap) would see an old `uploading` job on an IDLE printer, mark the LIVE job `failed`, and hold the printer, later enabling a phantom credit via the set-ready session-failed fallback. Fix: the `_activeUploads` guard now runs first, so a printer with an upload in flight is never treated as stale. This is the one ordering change TASK 6 authorized in scheduler.js; the PR 1 scheduler-lock and scheduler-recovery suites still pass untouched.

**Untracked FINISHED auto-dispatch onto an uncleared bed (server/scheduler.js).** `_handleFinished`'s no-tracked-job branch called `_dispatchToPrinter` immediately, so an operator-initiated (untracked) print finishing on an enrolled printer would auto-upload the next job onto the plate that was still on the bed. Fix: that branch now holds the printer (`is_held = 1`), writes a notification asking the operator to clear the bed and use Set Ready, and does not dispatch. Set Ready with no tracked job (reachable now that the hold satisfies finding 4's precondition) credits nothing and releases the hold, and recommission also releases it. This is the design's ask-the-operator answer to ambiguity.

**Double-fire analysis (restart, MQTT reconnect, poll flap).** Findings 4, 5, and the untracked-FINISHED fix all touch or gate `completed_qty`, so each was checked against the three replay vectors. Findings 4 and 5: the unique real-world event backing a credit is one operator confirming one physically completed plate on a held printer; `is_held` is the single-use token consumed inside the crediting transaction. A server restart does not re-fire a credit (no timer or heuristic drives set-ready; it is operator-initiated, and a resolved printer is `is_held = 0`). An MQTT reconnect cannot double-fire (set-ready is not called by reconnect logic; the failed-job fallback is gated on both `finished_at > startedAt` and no-newer-job). A poll flap cannot double-fire (poll ticks do not call set-ready; the endpoint's own duplicate handling returns 409). The untracked-FINISHED fix changes no `completed_qty` at all: it only sets a hold, so it is inert across all three vectors, and the subsequent Set Ready release is itself a no-credit path. Finding 6 changes no credit directly; it removes a race that could manufacture a stale `failed` row, and finding 4b independently blocks crediting such a row.

Testing. Two new suites drive the fixes. server/tests/audit-criticals-scheduler.test.js exercises the REAL scheduler (findings 6 and untracked-FINISHED) against an in-memory DB, the same idiom as scheduler-finished.test.js. server/tests/audit-criticals-setready.test.js mounts a replica of the fixed set-ready handler (the handler lives inside server/index.js's listen callback and cannot be imported under the house test idiom; the existing set-ready.test.js uses the same replica approach and had drifted, so this suite also reads the shipped server/index.js and asserts each fix marker is present to catch future drift). Every fix was shown failing first against unmodified logic. Full server suite green (32 suites, 472 tests; PR 1 scheduler suites unchanged and passing). No hardware validation applies (pure server-logic change; no driver code touched).

### Changes
- server/index.js: set-ready now returns 409 when the printer is not held (finding 4a); crediting plus hold release wrapped in one transaction with an inner is_held re-check for idempotency (finding 5); session-failed fallback query gained a NOT EXISTS newer-job scope guard (finding 4b).
- server/scheduler.js: `_dispatchToPrinter` consults the `_activeUploads` in-flight guard before the stale-job auto-fail (finding 6); `_handleFinished` no-tracked-job branch holds the printer and notifies instead of dispatching onto an uncleared bed.
- server/tests/audit-criticals-scheduler.test.js: new. Regression tests for finding 6 and the untracked-FINISHED hold, driving the real scheduler.
- server/tests/audit-criticals-setready.test.js: new. Regression tests for findings 4 and 5 plus the no-tracked-job release, against a faithful replica, with a source-invariant guard against replica drift.
- docs/api.md: set-ready section documents the 409 precondition, idempotency, the no-tracked-job release, and the session-failed fallback scope.
- docs/server.md: new "Scheduler FINISHED handling" section covering the tracked-job, session-failed, and untracked-job branches.

---

## 2026-07-12: Printer credential redaction (PR 3)

The internal fork's PR 3. The Phase 0 audit (docs/internal/audit-findings.md) found that every endpoint returning a printer row serialized the full row, including `api_key` (the Bambu LAN access code, which together with the serial number is direct MQTT/FTP control of the machine) and `serial_number`, in plaintext. It also found the same leak in the dashboard payload, the inline operator endpoints, the CSV import flagged-rows response, and the backup export, plus the client fetching and round-tripping the access code in the printer detail page. This closes those leaks: routes-credentials findings #3 (inline operator endpoints), #18 (every printers REST endpoint), the CSV echo note (printers.js:470) and the settings read-path note (settings.js:9); routes-files #23 (dashboard); client-leak #7 (PrinterDetail round-trip) and note 802 (text credential inputs); db-backup #36 (backup export credentials); and the serial-in-event-note finding (printers.js:210). PR 2's auth is the reason this is enforceable now: redaction cannot be bypassed by an unauthenticated request.

What changed for clients. Every response that returns a printer object now omits `api_key` entirely and adds `api_key_set` (`1` when a non-empty code is stored, else `0`), and masks `serial_number` to its last four characters as `"****" + tail` (`null` stays `null`). One shared sanitizer (server/lib/sanitize-printer.js) does this everywhere so there are no per-route copies to drift. Drivers are untouched: they keep reading the real `api_key`/`serial_number` straight from the DB, so nothing in server-to-printer communication changes. The scheduler still receives the raw row from the operator endpoints; only the HTTP response is redacted.

Writes are unchanged in spirit but gained a safety rule. Because the client never receives the access code, an edit form cannot round-trip it, so `PUT /api/printers/:id` now treats an empty-string `api_key` as "leave unchanged" (it previously would have overwritten the stored code with an empty string). A new `clear_api_key: true` body flag explicitly wipes the code when that is the intent. Serial-number changes are still logged to the printer event timeline, but the note no longer contains the raw serial (it references the printer instead). Historical event rows that already contain a serial are NOT rewritten: that is out of scope for this PR.

Backup default is now credentials-excluded, which is a breaking change for scripts. `GET /api/backup` no longer includes the `api_key` column on printer rows by default, so a routine backup is not a plaintext credential dump that gets stored or shared. Pass `?include_credentials=true` for a full disaster-recovery backup that keeps the codes (the endpoint is already admin-gated by PR 2). Serial numbers stay in the bundle unmasked because restore needs them and they are not a standalone secret. Restore accepts both shapes: a printer row with no `api_key` is restored with an empty code and counted in the new `printers_without_credentials` field of the restore response, so the operator is told which machines need their code re-entered. Anyone scripting `GET /api/backup` and expecting credentials must add the query flag.

Client. The printer detail page no longer fetches or displays the cleartext access code; it shows an "Access code: set / not set" indicator, the edit form's access-code and serial fields are write-only password/blank inputs (blank means unchanged), and a "Clear stored access code" action uses the `clear_api_key` flag behind a danger confirm. The Add Printer form's credential input is now `type="password"`. The CSV flagged-row response is rendered from the redacted shape. The backup UI gained an "Include printer access codes" checkbox (default unchecked).

Testing. New server/tests/redaction.test.js asserts no `api_key` and a masked serial on every printer-returning endpoint touched (list, single, create, update, decommission, complete-and-decommission, link-job, dashboard, CSV flagged rows), `api_key_set` correctness both ways, that PUT with `""` does not wipe the key while `clear_api_key` does, that the serial is not written into the event note, that the backup default excludes `api_key` while `include_credentials=true` includes it, and that restoring a credential-less backup sets `""` with the warning count. The inline operator endpoints (set-ready, recommission) cannot be mounted without server/index.js under the house test idiom, so their response shape is covered by the sanitizer's own unit tests plus the manual application of `sanitizePrinter` at their `res.json` sites. Full suite green (30 suites, 457 tests).

### Changes
- server/lib/sanitize-printer.js: new. Shared `sanitizePrinter`/`sanitizePrinters`/`maskSerial`; strips `api_key`, adds `api_key_set`, masks `serial_number`.
- server/routes/printers.js: redact every printer-returning response (list, decommissioned, single, create, update, decommission, complete-and-decommission, link-job); PUT treats empty-string `api_key` as keep and adds `clear_api_key`; serial-number event note no longer contains the raw serial; CSV import flagged rows echo `api_key_set` instead of the raw code.
- server/routes/dashboard.js: redact the printers array in the payload (stats still derived from the raw rows first).
- server/routes/backup.js: export excludes `api_key` by default, includes it with `?include_credentials=true`; restore defaults a missing `api_key` to `""` and returns `printers_without_credentials`.
- server/routes/settings.js: GET filters the response to the write-side allowlist (`ALLOWED_KEYS`).
- server/index.js: apply `sanitizePrinter` to the `res.json` of the inline set-ready and recommission operator endpoints (the scheduler still receives the raw row).
- client/src/pages/PrinterDetail.jsx: remove cleartext access-code display and round-trip; access-code/serial edit fields are write-only (password, blank means unchanged); add "Clear stored access code" action; render toast/confirm.
- client/src/pages/Settings.jsx: Add Printer credential field is `type="password"`; backup export "Include printer access codes" checkbox; restore warning for credential-less restores.
- server/tests/redaction.test.js: new. Full redaction coverage plus the sanitizer unit tests.
- docs/api.md: document `api_key_set`, serial masking, `clear_api_key`, the backup `include_credentials` flag and `printers_without_credentials`, settings read-path filtering, and the redacted CSV flagged-row shape.
- docs/web-app.md: printer detail access-code indicator and write-only edit fields, masked credential inputs, redacted CSV flagged rows, backup include-credentials checkbox.
